...
public class HungryPeer {
    ...
    // We discover RestoPeers via the module spec advertisement that was
    // published to advertise a RestoPeer Service
    private void discoverRestoServices() {
        int found = 0;                     // Count of RestoPeers found
        int count = 10;                    // Discovery retries
        ModuleSpecAdvertisement restoSrv;  // RestoPeer Service Advertisement
        ...
        while (count-- >0) {
            try {
                ...
                Enumeration ae =
                    disco.getLocalAdvertisements(DiscoveryService.ADV, "Name",
                                       "JXTASPEC:RestoNet:Service:*");
                ...
                if (ae != null && ae.hasMoreElements()) {
                    ...
                    while (ae.hasMoreElements()) {
                        restoSrv = (ModuleSpecAdvertisement) ae.nextElement();

                        // Extract the pipe advertisement from the
                        // service module spec adv. The pipe advertisement
                        // is used to connect to the service; it is
                        // part of the Param section of the advertisement
                        restoPeerAdvs.addElement(restoSrv.getPipeAdvertisement());
                        ++found;
                    }
                ...
                // Send a remote discovery request to search for more
                // RestoPeer service advertisements
                disco.getRemoteAdvertisements(null,
                                DiscoveryService.ADV,
                                "Name", "JXTASPEC:RestoNet:Service:*", 5, null);
        ...
        System.out.println("Found " + found + " RestoPeer Service(s)");
    }
